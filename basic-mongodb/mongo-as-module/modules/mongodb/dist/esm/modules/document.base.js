...
// };
export const error = (msg, originError) => {
...
export class DaoBase {
    ...
    */
    ...
    _getObjectId = (id) => (typeof id === 'string' ? new ObjectId(id) : id);
    // 23. string과 ObjectId가 혼재된 배열에서 string 타입 값만 ObjectId 타입으로 변경함
    _getObjectIdList = (idList) => idList.map((x) => (typeof x === 'string' ? new ObjectId(x) : x));
    // 24.
...
export const fromDate = (date) => new Date(`${date}T00:00:00.000`);
export const toDate = (date) => new Date(`${date}T23:59:59.999`);
export const getFromToQuery = (date) => { };
