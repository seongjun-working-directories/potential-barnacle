...
exports.getFromToQuery = exports.toDate = exports.fromDate = exports.getCurrentDate = exports.DaoBase = exports.error = void 0;
...
class DaoBase {
    ...
    */
    ...
    dateQueryUtil = {
        ...
        fromDate: (date) => new Date(`${date}T00:00:00.000`),
        toDate: (date) => new Date(`${date}T23:59:59.999`),
        getFromToQuery: (date) => {
            return {
                $gte: getCurrentDate((0, exports.fromDate)(date[0])),
                $lte: getCurrentDate((0, exports.toDate)(date[1])),
            };
        },
        getDayToQuery: (date) => {
            return {
                $gte: getCurrentDate((0, exports.fromDate)(date)),
                $lte: getCurrentDate((0, exports.toDate)(date)),
            };
        },
    };
    /*
    Constructor
    */
    constructor(client, userInfo, collectionName) {
        this._client = client;
    ...
    */
    ...
    // 7.
        ...
        if (this._linkInstance) {
            this._linkInstance.forEach(async (targetInstance) => {
                if (typeof targetInstance.link === 'function') {
    ...
    _getCollection = async () => {
        try {
            ...
            if (!db)
                throw (0, exports.error)('no database');
            // db.collection : Returns a reference to a MongoDB Collection.
            const collection = await db.collection(this._collectionName);
    ...
    _findOne = async (filter, options) => {
        try {
            ...
            if (!this._isKeepInstance)
                await this._close();
        }
    };
    // 12. collection.find().toArray();
    _find = async (filter, options) => {
        try {
            const collection = this._collection ?? (await this._getCollection());
    ...
    _countDocument = async (filter = {}, options) => {
        try {
            const collection = this._collection ?? (await this._getCollection());
            const count = options
    ...
    _insertMany = async (docs, options = {}) => {
        try {
            const collection = this._collection ?? (await this._getCollection());
    ...
    _updateOne = async (filter, update, options = {}) => {
        try {
            const collection = this._collection ?? (await this._getCollection());
    ...
    _updateMany = async (filter, update, options = {}) => {
        try {
            const collection = this._collection ?? (await this._getCollection());
    ...
    _deleteOne = async (filter, options = {}) => {
        try {
            const collection = this._collection ?? (await this._getCollection());
    ...
    _deleteMany = async (filter, options = {}) => {
        try {
            const collection = this._collection ?? (await this._getCollection());
    ...
    _getCreateInfoDocument = (docList) => {
        // OptionalUnlessRequiredId 타입은 다음의 특징을 갖고 있음
        // - _id 라는 속성을 무조건 포함함
        // - 모든 속성은 string 형태로 저장되어 있음
    ...
    _getUpdateInfoDocument = (docList) => {
    ...
    _getObjectId = (id) => (typeof id === 'string' ? new mongodb_1.ObjectId(id) : id);
    // 23. string과 ObjectId가 혼재된 배열에서 string 타입 값만 ObjectId 타입으로 변경함
    _getObjectIdList = (idList) => idList.map((x) => (typeof x === 'string' ? new mongodb_1.ObjectId(x) : x));
    // 24.
    getItemList = async (options) => {
        const list = await this._find({}, options);
        return list;
    };
    // 25. string 'id' -> ObjectId '_id' -> findOneById(filter가 id, options는 주입받음)
    getOneItemById = async (id, options) => {
        return await this._findOne({ _id: this._getObjectId(id) }, options);
    };
    // 26. (string|ObjectId)[] 'idList' -> ObjectId[] 'idList' -> _find(filter가 idList, options는 주입받음)
    getManyItemById = async (idList, options) => {
    ...
    updateManyItem = async (updateItemList, options) => {
    ...
    deleteOneItemById = async (id, options) => {
    ...
    deleteManyItemById = async (idList, options) => {
...
const fromDate = (date) => new Date(`${date}T00:00:00.000`);
exports.fromDate = fromDate;
const toDate = (date) => new Date(`${date}T23:59:59.999`);
exports.toDate = toDate;
const getFromToQuery = (date) => { };
exports.getFromToQuery = getFromToQuery;
//# sourceMappingURL=document.base.js.map